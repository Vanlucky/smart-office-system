	...
	: CDialogEx(IDD_MFC_SERIAL_DIALOG, pParent)
{
	...
	m_bitmap->LoadBitmap(IDB_BMP_BK);
...
BEGIN_MESSAGE_MAP(CMFCserialDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BTN_ABOUT, &CMFCserialDlg::OnBnClickedBtnAbout)
	ON_WM_ERASEBKGND()
END_MESSAGE_MAP()
...
BOOL CMFCserialDlg::OnInitDialog()
{
	...
	m_chart.GetTitle()->AddString(_T("传感器监视曲线"));
	...
	pLineSerieRef->SetWidth(4);
	//pLineSerieRef->SetSmooth(true);
	pLineSerieRef->SetColor(RGB(0x0, 0x0, 0xFF));
	pLineSerieRef->SetSeriesOrdering(poXOrdering);
	pLineSerieRef->SetName(_T("转速"));

	pLineSerie->SetWidth(4);
	pLineSerie->SetColor(RGB(0xFF, 0, 0));
	pLineSerie->SetSeriesOrdering(poXOrdering);
	pLineSerie->SetName(_T("温度"));


	m_serialport.EmumSerialsToBox(m_combobox);
	//Serial.print(F("EEEEE"));
	//Serial.print(F("R F"));
	//Serial.print(speed);
	//Serial.print(F("T"));
	//Serial.print(t);
	...
	//Serial.print(F("EEEEE"));
	//string str;
	//sprintf(str.c_str(),"" "R F%dT%fL%dl%fE", 0, 0, 0, 0)

	SetDlgItemText(IDC_EDIT2, _T("9600"));

	//SetDlgItemText(IDC_EDIT_DIS, _T("0"));
	//SetDlgItemText(IDC_EDIT_P, _T("0"));
	//SetDlgItemText(IDC_EDIT_I, _T("0"));
	//SetDlgItemText(IDC_EDIT_D, _T("0"));
	//SetDlgItemText(IDC_EDIT_P1, _T("0"));
	//SetDlgItemText(IDC_EDIT_D1, _T("0"));
	//append_message(itoa(sizeof(bundle)));
	//char str[32];
	//sprintf(str, "R F%03dT%6.2fL%03dl%6.2f", 0, 1.9, 10, 10.98);
	//int a1, a2;
	//float b1, b2;
	//sscanf(str, "R F%dT%fL%dl%f", &a1, &b1, &a2, &b2);

	//char str1[8];
	//string ss(str1);
	
	//char str2[20];
	//sprintf(str, "R  F%03dT%6.2fL%03dl%6.2f", speed, t, light, distance);
	//Serial.print(str);
...
afx_msg LRESULT CMFCserialDlg::OnSendMsgHandler(WPARAM w, LPARAM l) {
	...
	//char buf[20];
	sscanf((char*)raw, "R  F%dT%fL%dl%f", &m_speed, &m_t, &m_light, &m_distance);
	//sscanf((char*)raw, "%*[^R]R F%dT%fL%dl%fE%*[^R]", &m_speed, &m_t, &m_light, &m_distance);
	//strw.Format(_T("%d   %f   %d   %f"), m_speed, m_t, m_light, m_distance);
	//append_message(strw);

	CString tmp;
	tmp.Format(_T("%d"), m_speed);
	GetDlgItem(IDC_EDIT_P)->SetWindowText(tmp);
	tmp.Format(_T("%.2f"), m_t);
	SetDlgItemText(IDC_EDIT_I, tmp);
	tmp.Format(_T("%d"), m_light);
	SetDlgItemText(IDC_EDIT_D, tmp);
	tmp.Format(_T("%.2f"), m_distance);
	GetDlgItem(IDC_EDIT_DIS)->SetWindowText(tmp);
...
void CMFCserialDlg::OnBnClickedUpdate()
{
	...
	GetDlgItemText(IDC_EDIT_P1, P);
	//GetDlgItemText(IDC_EDIT_I, I);
	GetDlgItemText(IDC_EDIT_D1, D);
	...
	char update[12];
	sprintf(update, "SF%dL%d\r\n", speed, light);


	int count = m_serialport.SendData((unsigned char*)update,
		strlen(update));
	CTime tm = CTime::GetCurrentTime();
	if (count == strlen(update)) {
...
void CMFCserialDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (nIDEvent == TIMER_EVENT) {
		...
		m_serialport.SendData((unsigned char*)&request,
			sizeof(char)*2);
	}
...
void CMFCserialDlg::OnBnClickedBtnAbout()
{
	CAboutDlg dlg;
	dlg.DoModal();
}
...
BOOL CMFCserialDlg::OnEraseBkgnd(CDC* pDC)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	CDC dcMem;
	CClientDC dc(this);
	...
	CRect rect;
	GetClientRect(&rect);
	pDC->FillRect(rect, &m_brushbk);//充满整个客户区
	return 0;
